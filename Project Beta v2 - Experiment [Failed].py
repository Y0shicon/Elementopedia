...
def create_graph_win(choice):
	if choice == drop_options2[0] or choice == drop_options1[0]:
		...
		keyword = find_graph_dic(choice)
		global group
		global graph_win
		graph_win = Toplevel()	
		graph_win.title(choice.split()[0:2])
		i = 0
		for images in all_graphs:
			if keyword in images:
				print(images)
				Label(graph_win, image = ImageTk.PhotoImage(Image.open('Graphs/' + images).resize(int(width_value//3),int(height_value//3)), Image.ANTIALIAS()).pack()
				print(images, 'done')
				i += 1
...
global choice_graphs
#choice_graphs = StringVar()
#choice_graphs.set('general')

#Button(root, text = 'General', command = lambda :setGraph(drop1, drop1_but)).grid(row = 2, columnspan = 5, column = 3)
#Button(root, text = 'Specific', command = lambda :setGraph(drop2, drop2_but)).grid(row = 3, columnspan = 5, column = 3)
#graph_radio1 = Radiobutton(root, text = 'Observe a general periodic trend', variable = choice_graphs, value = 'general')
#graph_radio2 = Radiobutton(root, text = 'Observe a periodic trend in specific groups', variable = choice_graphs ,value = 'specific')
#graph_radio1.bind('<Button-1>', lambda event : setGraph(drop1, drop1_but))c
#graph_radio2.bind('<Button-1>', lambda event : setGraph(drop2, drop2_but))

#graph_radio1.grid(row = 2, column = 3, columnspan =5)
#graph_radio2.grid(row = 3, column = 3, columnspan = 5)
